...
impl ButtonContainer {
    ...
    pub fn is_timer(&self, event: Event) -> bool {
        self.long_pressed_timer.is_expired(event)
    }
...
impl ButtonController {
    ...
    fn handle_long_press_timers(&mut self, event: Event) -> Option<ButtonPos> {
        if self.left_btn.is_timer(event) {
            return Some(ButtonPos::Left);
        }
        if self.middle_btn.is_timer(event) {
            return Some(ButtonPos::Middle);
        }
        if self.right_btn.is_timer(event) {
...
impl IgnoreButtonDelay {
    ...
    pub fn handle_timers(&mut self, event: Event) {
        if self.left_clickable_timer.is_expired(event) {
            self.left_clickable = false;
        }
        if self.right_clickable_timer.is_expired(event) {
...
impl Component for AutomaticMover {
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: Event) -> Option<Self::Msg> {
        if self.timer.is_expired(event) && self.moving_direction.is_some() {
