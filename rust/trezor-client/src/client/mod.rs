...
impl Trezor {
    ...
    pub fn recover_device(
        ...
        if dry_run {
            req.set_type(protos::recovery_device::RecoveryType::DryRun);
        } else {
            req.set_type(protos::recovery_device::RecoveryType::NormalRecovery);
        }
        req.set_input_method(
            protos::recovery_device::RecoveryDeviceInputMethod::ScrambledWords,
        );
